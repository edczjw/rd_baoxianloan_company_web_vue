<template>
  <div class="goods-wapper">
    ...
    <el-card class="box-card">
          ...
          <el-row :gutter="24">
            ...
            <el-col :span="8">
              <el-form-item label="验证码" prop="verifyCode">
                <el-input class="ell" placeholder="请输入验证码" v-model.trim="checkform.verifyCode">
                  <template slot="prepend">
      ...
      <div class="dibu">
        <div class="checkbox">
          <el-checkbox prop="agreementStatus" v-model="agreementStatus">
            已同意并愿意接受
            <a
              href="https://yixiangweilai.oss-cn-shenzhen.aliyuncs.com/jiekuanxieyi/%E5%80%9F%E6%AC%BE%E5%90%88%E5%90%8C%EF%BC%88%E4%BC%81%E4%B8%9A%E7%BA%BF%E4%B8%8A%E7%89%88%EF%BC%89%EF%BC%8D%E6%98%93%E4%BA%AB%E9%A1%B9%E7%9B%AE%EF%BC%8D%E5%AE%9A%E7%A8%BF1.0%20%E4%B8%8D%E5%8E%BB%E6%A1%86%284%29%281%29%282%29.pdf"
              target="_blank"
            >《借款协议》</a>
          </el-checkbox>
        </div>
  ...
  methods: {
    ...
    submit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (
            this.availableCredit < this.checkform.applyLimit ||
            this.checkform.applyLimit < 1000 ||
            this.checkform.applyLimit > 500000
          ) {
            this.$confirm(
              "申请借款金额不得高于当前可用额度，并且一次不得低于1000元，不得高于500000元.",
              "提示",
              {
      ...
      this.$axios({
        ...
        response => {
          ...
          if (this.checkform.agreementUrl == "") {
            this.checkform.agreementUrl =
              "http://yixiangweilai.oss-cn-shenzhen.aliyuncs.com/" + storeAs;
          } else {
            this.checkform.agreementUrl =
              this.checkform.agreementUrl +
              ",http://yixiangweilai.oss-cn-shenzhen.aliyuncs.com/" +
              storeAs;
          }
